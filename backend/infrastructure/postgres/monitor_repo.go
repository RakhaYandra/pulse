...
const monitorCols = `id,user_id,name,url,method,interval_seconds,timeout_seconds,failure_threshold,recovery_threshold,status,is_active,last_checked_at,created_at`
...
func scanMonitor(s interface{ Scan(...any) error }) (domain.Monitor, error) {
	...
	err := s.Scan(&m.ID, &m.UserID, &m.Name, &m.URL, &m.Method, &m.IntervalSeconds,
		...
		&status, &m.IsActive, &last, &created)
	if err != nil {
...
func (r MonitorRepo) Create(ctx context.Context, m domain.Monitor) error {
	_, err := r.DB.ExecContext(ctx, `INSERT INTO monitors(id,user_id,name,url,method,interval_seconds,timeout_seconds,failure_threshold,recovery_threshold)
...
func (r MonitorRepo) Update(ctx context.Context, m domain.Monitor) error {
	res, err := r.DB.ExecContext(ctx, `UPDATE monitors SET name=$1,url=$2,interval_seconds=$3,timeout_seconds=$4,
		...
		m.Name, m.URL, m.IntervalSeconds, m.TimeoutSeconds, m.FailureThreshold, m.RecoveryThreshold, m.ID, m.UserID)
	if err != nil {
		return err
	}
